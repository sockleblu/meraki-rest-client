...
describe MerakiProvisions do
    ...
    describe '#getMXL3Rules' do
        context 'when network id' do
            it 'responds with 200 ' do
                response = double

                allow(response).to receive(:code).and_return(200)
                allow(@meraki).to receive(:get_mx_l3_rules).and_return(response)

                rep = @meraki.get_mx_l3_rules(network_id: :good_id)
            ...
            it 'retrieves a valid json object of L3 rules' do
                ...
                allow(response).to receive(:body).and_return(@mx_l3_rules)
                allow(@meraki).to receive(:get_mx_l3_rules).and_return(response)
                rep = @meraki.get_mx_l3_rules(network_id: :good_id)

                expect(@meraki.valid_json?(@mx_l3_rules)).to eq true
                expect(rep.body).to eq @mx_l3_rules
            end
        end
        ...
        context 'when network id is invalid' do
            it 'responds with 404 code' do
                ...
                rep = @meraki.get_mx_l3_rules(network_id: :bad_id)
        ...
        context 'when called without providing network id' do
            it 'responds with ArgumentError' do
                expect { @meraki.get_mx_l3_rules }.to raise_exception(ArgumentError)
            end
        end

        context 'when called with proper network id' do
            it 'does not respond with ArgumentError' do
                allow(@meraki).to receive(:get_mx_l3_rules).and_return(@mx_l3_rules)
                expect do
                    @meraki.get_mx_l3_rules(network_id: :id)
    ...
    describe '#updateL3Rules' do
        context 'when network id and json valid' do
            it 'responds with 200 ' do
                ...
                allow(response).to receive(:code).and_return(200)
                allow(@meraki).to receive(:update_mx_l3_rules).and_return(response)

                rep = @meraki.update_mx_l3_rules(network_id: :good_id,
                                                 json: :good_json)
        ...
        context 'when network id or ssid is invalid' do
            it 'responds with 404 code' do
                ...
                rep = @meraki.update_mx_l3_rules(network_id: :bad_id,
                                                 json: @mx_l3_rules)
        ...
        context 'when called without network id and valid json' do
            it 'responds with ArgumentError' do
                expect { @meraki.update_mx_l3_rules }.to raise_exception(ArgumentError)
                expect { @meraki.update_mx_l3_rules(network_id: :id) }.to raise_exception(ArgumentError)
                expect do
                    @meraki.update_mx_l3_rules(network_id: :id,
                                               route_id: :route)
                end.to raise_exception(ArgumentError)

                allow(@meraki).to receive(:update_mx_l3_rules).and_return(@mx_l3_rules)
                expect do
                    @meraki.update_mx_l3_rules(network_id: :id,
                                               json: @mx_l3_rules)
                end.to_not raise_exception
