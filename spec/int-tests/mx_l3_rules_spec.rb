...
describe MerakiProvisions do
    ...
    feature 'Get a networks firewall rules' do
        it 'queries Meraki for a single network by ID' do
            response = @meraki.get_mx_l3_rules(network_id: 'L_569142402908947018')
    ...
    feature 'Update the attributes of a network' do
        it 'updates the attributes of a networks using json' do
            json = File.open(@fixed_loc + 'mx_l3_update.json').read

            response = @meraki.update_mx_l3_rules(network_id: 'L_569142402908947018', json: json)
